...
const chatbotAPI = async (req, res) => {
  try {
    ...
    const knowledgeBase = {
      ...
      "who is the principal?": "The current principal of RGMCET is Dr. T. Jayachandra Prasad.",
      "What is the position of the RGM college in the Andhra Pradesh?": "The position of the RGMCET is 201-300.",
      "How many students are offered with the placements in 2022?": "1448 students were placed in 2022.",
      "Which company offered too many offers to the student in RGMCET?": "Wipro, Bangalore offered the most placements.",
      "What is the highest package in RGMCET?": "The highest package offered at RGMCET is 12 LPA.",
      "Which department students got the highest packages in the RGMCET?": "CSE and ECE students received the highest packages.",
      "What is the rating for the RGMCET?": "RGMCET has an A+ rating.",
      "Why choose RGMCET?": "RGMCET was established in 1995 with a motto 'Education for Peace' and a mission to produce excellent Engineers with social awareness meeting global standards. It also aims to achieve excellence in research and consultancy in the field of Science and Technology.",
      "which facilities are available in the RGMCET?": "Facilities include a Library, Boys Hostel, Girls Hostel, Cafeteria, and Ground.",
      "who is the chairman of the RGM college?": "The chairman of RGMCET is Dr. M. Santhiramudu.",
      "Which departments are available in the RGM college?": "Departments include CSE, ECE, CSE-DS, CSE-AIML, CSE-BS, CSE-CS, EEE, MECH, CIVIL, and MBA.",
    };

    // Check if the message matches any predefined queries
    const lowerCaseMessage = message.toLowerCase();
    if (knowledgeBase[lowerCaseMessage]) {
      const reply = knowledgeBase[lowerCaseMessage];

      // Speak the reply using the Web Speech API
    ...
    const response = await axios.post(
      ...
      {
        contents: [{ role: "user", parts: [{ text: `Provide information about ${message} related to the RGM College Nandyal AP in 20 words - for analyzation refer ${JSON.stringify(knowledgeBase)}` }] }]
      },
      {
        headers: {

...
const analyzeEmoji= async(req, res) => {
    const { postContent } = req.body;
    console.log("entered int emoji");
    if (!postContent) {
    ...
    try {
        const response = await axios.post(
            ...
            {
                contents: [{ parts: [{ text: `Analyze the sentiment of this text and suggest 3 relevant emojis(not symbols or characters)only emojis: ${postContent}` }] }]
            },
        ...
        res.json({ suggestedEmojis: aiResponse.trim()});

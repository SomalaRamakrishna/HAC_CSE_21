...
const StatisticsPage = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const response = await axios.get("https://hackthon-cse-25.onrender.com/api/users/stats",
        {
  ...
  return (
    <div className="statistics-container">
      <h1 className="stats-title">
       {/*  <Sparkles size={30} color="darkBlue"/> */} Statistics & Insights
      </h1>

      {/* Live Stats Counter */}
      <div className="live-stats">
        <h2><Zap size={25} color="darkBlue"/> Live Likes Count: {liveCounter}</h2>
      </div>

      {/* Overview Cards */}
      <div className="stats-grid">
        <div className="stats-card">
          <ThumbsUp size={40} color="darkBlue"/> 
          <h3>{lastWeekStats.likes} Likes</h3>
          <p>Last Week</p>
        </div>

        <div className="stats-card">
          <Users size={40} color="darkBlue"/> 
          <h3>{lastWeekStats.followers} New Followers</h3>
          <p>Last Week</p>
        </div>

        <div className="stats-card">
          <TrendingUp size={40} color="darkBlue"/> 
          <h3>Ranking <UserRank userId={userId}/></h3>
          <p>Your Ranking level</p>
        </div>

        <div className="stats-card">
          <Flame size={40} color="darkBlue"/> 

...
const ChatArea = ({ activeChat, setActiveChat ,isMobile,setIsShow}) => {
 ...
 const [loading, setLoading] = useState(true);
  ...
  return (
    <div className="chat-window">
      <div>
        {isMobile && 
        <ArrowLeft  onClick={()=>{setActiveChat(null);localStorage.removeItem("activeChat");setIsShow(false);}}/>
        }</div>
      ...
      {activeChat ? (<div className="message-input">
        <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Write a message..." />
        <button onClick={sendMessage}><Send /></button>
      </div>):<span style={{color:"red"}}> *Pelase select any follower or following Member</span>}
    </div>
  );
};
